...
async def get_repo_details(session, owner, repo):
    ...
    latest_release = None
    if sorted and releases and isinstance(releases, list) and len(releases) > 0:
...
async def main():
    ...
    # Fetch all repositories of organization 'ticohq'
    org_repos_url = "https://api.github.com/orgs/ticohq/repos?per_page=100"
    print(f"Fetching repos list from {org_repos_url}...", file=sys.stderr)
    repos_list = await github_request(session, org_repos_url)
    
    if not repos_list or not isinstance(repos_list, list):
        print("Error: Could not fetch repositories for ticohq org.", file=sys.stderr)
        await close_clients()
        sys.exit(1)
        
    print(f"Found {len(repos_list)} repositories for ticohq", file=sys.stderr)
    
    generated_entries = []
    
    for repo_info in repos_list:
        repo_name = repo_info.get("name")
        print(f"Fetching details for {repo_name}...", file=sys.stderr)
        details = await get_repo_details(session, "ticohq", repo_name)
        if details:
    ...
    print(f"Successfully added {added_count} new entries to {manual_path} (total entries: {len(manual_entries)})", file=sys.stderr)
    await close_clients()
