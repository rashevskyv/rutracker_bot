...
def translate_ru_to_ua_google(text):
    ...
    translated_text = translated_text.replace(" :", ":").replace(":<a", ": <a").replace("|", " | ").replace("</a>(", "</a> (").replace("</b><a", '</b> <a').replace("<a", " <a").replace("</a>", "</a> ").replace(' , ', ", ").replace("  ", " ").replace("# ", "#").replace("( <a", "(<a").replace("</a> )", "</a>)").strip()
