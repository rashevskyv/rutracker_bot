...
async def send_digest():
    ...
    is_forced = os.environ.get('FORCE_TASK') == 'run_homebrew_digest'
    if not IS_TEST_MODE and not is_forced:
        if current_time - last_run_time < timedelta(hours=20):
            logger.info(f"Homebrew digest was already sent recently (last run: {last_run_time}). Skipping.")
            return
    ...
    if IS_TEST_MODE:
        ...
        try:
            for group in TEST_GROUPS:
                ...
                logger.info(f"Sending test homebrew digest to {group.get('group_name', 'Unknown')} (chat_id: {chat_id}, topic_id: {topic_id}, translate: {translate_to_ua})")
